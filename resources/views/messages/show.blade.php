...
<a href="{{ route('messages.index') }}" class="btn btn-secondary mt-3">Kembali ke Daftar Pesan</a>
    ...
    <!-- Cek apakah user login adalah penerima -->
    @if(Auth::user()->email == $message->recipient_email)
        <form action="{{ route('messages.store') }}" method="POST">
            ...
            <input type="hidden" name="recipient_email" value="{{ $message->sender }}"> <!-- Pengirim asli menjadi penerima balasan -->
